    ...
    methods: {
      ...
      login(){
        ...
        return http_sys
          ...
          .then(data => {
              ...
              {
                // localStorage.setItem("token", "Bearer "+data.data.data.accessToken);
              
                _this.loginSuccess("Bearer "+data.data.data.accessToken,data.data.data.userInfo,redirectUrl)
              }else{
                _this.$message(data.data.msg||data.data.errMessage);
      ...
      // }
       loginSuccess(token,user,redirectUrl){
        ...
        // 在实例已创建后修改默认值
        http_sys.request.defaults.headers.common['Authorization'] =token;
        window.sessionStorage.setItem("user-info", JSON.stringify(user));
        this.setUserInfo(user);
        // this.$http.defaults.headers.common['authSid'] = sid;
        this.loadMenuList();
